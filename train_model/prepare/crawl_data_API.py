...
def download_audio(word, accent, output_path):
    """Download audio file for a word using Free Dictionary API"""
    try:
        clean_word = word.strip("'")
        
        # Query Free Dictionary API to get audio URL
        response = requests.get(
            f"{DICT_API_BASE}/{clean_word.lower()}",
            timeout=10
        )
        ...
        if response.status_code == 200:
            try:
                ...
                audio_url = us_url or uk_url or any_url
                if not audio_url:
                    return False
